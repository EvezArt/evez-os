#!/usr/bin/env python3
"""EVEZ-OS CLI — Visual Cognition Layer v1.2.0
...
Quad-panel mode (--panels 4):
  Renders a 4th panel per frame with SAN narrator triad:
  EXECUTOR | TRICKSTER | GAME_BUILDER
  + confidence tier, smugness ratio, falsifier status, per frame.
"""
...
_SAN_VOICES = {
    "EXECUTOR":     "{lobby} probe step {step} | hypothesis pending falsification | truth_plane=PENDING",
    "TRICKSTER":    "single-vantage {lobby} data -> THEATRICAL if no second probe. Falsifier: run from second machine.",
    "GAME_BUILDER": "Sf: if {lobby} probe unreplicated -> mission M-{lobby}-DUAL seeded. Smugness feeds the queue.",
}
...
def _maybe_make_demo_image(out_dir: Path, step: int, seed: int) -> Optional[str]:
    ...
    for _ in range(6):
        x1 = rng.randint(0, W-40); y1 = rng.randint(0, H-40)
        x2 = x1 + rng.randint(30, 160); y2 = y1 + rng.randint(30, 160)
        col = (rng.randint(40, 255), rng.randint(40, 255), rng.randint(40, 255))
        d.rectangle([x1, y1, x2, y2], outline=(230, 240, 255), width=2, fill=col)
    cx = rng.randint(80, W-80); cy = rng.randint(80, H-80)
...
def cmd_play(args: argparse.Namespace) -> None:
    ...
    print(f"\n{'='*68}")
    print(f" EVEZ Play Engine | seed={args.seed} | steps={args.steps} | mode={mode} | out={spine_path}")
    ...
    for i in range(args.steps):
        ...
        san_entries = [
            {
                "narrator":     "EXECUTOR",
                "text":         _SAN_VOICES["EXECUTOR"].format(lobby=lobby, step=i),
                "confidence":   "PENDING",
                "falsifier":    f"Run {lobby} probe from second vantage and append result.",
                "smugness_ratio": round(len(claim.split()) / 10.0, 2),
            },
            {
                "narrator":     "TRICKSTER",
                "text":         _SAN_VOICES["TRICKSTER"].format(lobby=lobby),
                "confidence":   "PENDING",
                "falsifier":    "Second-vantage probe result appended to spine.",
                "smugness_ratio": round(len(claim.split()) / 10.0, 2),
            },
            {
                "narrator":     "GAME_BUILDER",
                "text":         _SAN_VOICES["GAME_BUILDER"].format(lobby=lobby),
                "confidence":   "PENDING",
                "falsifier":    f"Mission M-{lobby}-DUAL executed and spine-logged.",
                "smugness_ratio": None,
            },
        ]
        event: Dict[str, Any] = {
    ...
    print(f"\n[OK] {args.steps} spine entries written.")
    print(f"   Lint:      python3 tools/evez.py lint --input {spine_path}")
    print(f"   Triptych:  python3 tools/evez.py visualize-thought --input {spine_path}")
    print(f"   Quad:      python3 tools/evez.py visualize-thought --input {spine_path} --panels 4\n")


def _print_lint_result(res: LintResult, verbose: bool) -> None:
    status = "[OK]" if res.violations == 0 else "[FAIL]"
...
def cmd_visualize(args: argparse.Namespace) -> None:
    ...
    panels  = getattr(args, "panels", 3)
    out = visualize_spine(
        spine_path=spine_path, out_dir=out_dir,
        title=args.title, fps=args.fps, max_steps=args.max_steps, panels=panels,
    )
    mode = "quad" if panels >= 4 else "triptych"
    print(f"[OK] Artifacts written to: {out.out_dir}  [{mode} mode]")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    play.add_argument("--with-images", action="store_true",
                      help="Also write synthetic observation PNGs (requires Pillow)")
    play.add_argument("--mode", default="standard", choices=["standard","spicy","forensic","synesthetic","hyperloop"], help="Narrative mode")
    ...
    vz.add_argument("--panels",    type=int, default=3,
                    help="3=triptych (default), 4=quad (adds SAN narrator panel per frame)")
